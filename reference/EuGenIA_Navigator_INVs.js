...
// #####################################################
// This function retrieves the latest investigation row
// from BigQuery by trin_id.
// Input: JSON with trin_id (e.g. {"trin_id": "INV-0006018"})
// Returns: the most recent row (by updated_date) for that trin_id
// #####################################################
function investigationByTrinId(ticket, params) {
  var rawInput = params.get(0);
  var trinId = "";
  var result = { content: "", logs: "" };

  // Log all params received to debug MCP calls
  ticket.addOutput("investigationByTrinId: params.length=" + params.length);
  for (var i = 0; i < params.length; i++) {
    ticket.addOutput("investigationByTrinId: params[" + i + "]=" + params.get(i));
  }

  ticket.addOutput("investigationByTrinId: rawInput=" + rawInput);

  // Support plain string, {"trin_id":"..."} or MCP format {"input":"..."}
  try {
    var jsonObject = JSON.parse(rawInput);
    trinId = jsonObject.trin_id || jsonObject.input || "";
  } catch (e) {
    // Not JSON — treat the raw input directly as the trin_id
    trinId = rawInput ? rawInput.trim() : "";
  }

  ticket.addOutput("investigationByTrinId: trinId=" + trinId);

  if (trinId === undefined || trinId === null || trinId === "") {
    result.logs = "ERROR: trin_id parameter is required";
    ticket.addOutput("investigationByTrinId: " + result.logs);
    ticket.getResult().setObject(JSON.stringify(result));
    ticket.getResult().setResult(TheSysModuleFunctionResult.RESULT_NOK);
    return;
  }

  var sql_query = 'SELECT * FROM `networkanalytics-p-292818.trin.investigation` ' +
      'WHERE day_part > "2020-01-01" ' +
      'AND trin_id = "' + trinId + '" ' +
      'QUALIFY ROW_NUMBER() OVER (PARTITION BY trin_id ORDER BY updated_date DESC) = 1';

  ticket.addOutput("investigationByTrinId: sql_query=" + sql_query);
  ...
  if (!ModuleUtils.waitForTicketsSuccess(runTicketGCP)) {
    ticket.addOutput("investigationByTrinId: Query falhou");
    result.logs = "ERROR: Query failed for trin_id " + trinId;
    ticket.getResult().setObject(JSON.stringify(result));
    ticket.getResult().setResult(TheSysModuleFunctionResult.RESULT_NOK);
    return;
  }
  ...
  ticket.addOutput("investigationByTrinId: raw GCP response=" + JSON.stringify(data_runTicketGCP).substring(0, 500));
  ...
  result.logs = "Found " + data_runTicketGCP.Result.length + " record(s) for trin_id " + trinId;
  ticket.addOutput("investigationByTrinId: " + result.logs);
  ticket.addOutput("investigationByTrinId: result=" + JSON.stringify(result).substring(0, 500));
  ticket.getResult().setObject(JSON.stringify(result));
  ticket.getResult().setResult(TheSysModuleFunctionResult.RESULT_OK);
}
...
function startModule() {
  ...
  var functions = [
    {
     name: "investigationByTrinId",
     path: "/ai/sara/investigationsByTrinId",
     parameters: "input*string,extra*string",
     description: "Function for getting the latest investigation row by trin_id from BigQuery @Authors:Sara@"
    }
    /*{
     name: "helloWorld",
